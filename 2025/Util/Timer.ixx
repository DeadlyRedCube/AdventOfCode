...
public:
  ...
  u64 MillisecondsSinceLastCheckpoint() const noexcept
    { return u64(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - lastCheckpointTime).count()); }
  ...
  {
    auto now = std::chrono::high_resolution_clock::now();
    auto result = u64(std::chrono::duration_cast<std::chrono::milliseconds>(now - lastCheckpointTime).count());
    lastCheckpointTime = now;
    return result;
  }
