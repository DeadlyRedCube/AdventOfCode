...
export
{
  ...
  {
    Array2D<char> ary { ssz(v[0].length()), ssz(v.size()) };
    for (ssz y = 0; y < ary.Height(); y++)
    {
      for (ssz x = 0; x < ary.Width(); x++)
        { ary[{x, y}] = v[usz(y)][usz(x)]; }
    }
  ...
  s32 AsS32(std::string_view s)
  {
    char *end;
    return std::strtol(s.data(), &end, 10);
  }

  s64 AsS64(std::string_view s)
  {
    char *end;
  ...
  public:
    ...
    void Fill(const T &v)
    {
      for (ssz i = 0; i < dims[0] * dims[1] * dims[2]; i++)
        { data[i] = v; }
    }
